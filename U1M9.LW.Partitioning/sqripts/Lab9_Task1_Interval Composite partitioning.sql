...
PARTITION PART_2 VALUES LESS THAN (TO_DATE('01/07/2013', 'DD/MM/YYYY')),
PARTITION PART_3 VALUES LESS THAN (TO_DATE('01/01/2014', 'DD/MM/YYYY')),
PARTITION PART_4 VALUES LESS THAN (TO_DATE('01/07/2014', 'DD/MM/YYYY')),
PARTITION PART_5 VALUES LESS THAN (TO_DATE('01/01/2015', 'DD/MM/YYYY')),
PARTITION PART_6 VALUES LESS THAN (TO_DATE('01/07/2015', 'DD/MM/YYYY'))
)
...
SELECT * FROM calendar PARTITION (PART_4);
SELECT * FROM calendar PARTITION (PART_5);
SELECT * FROM calendar PARTITION (PART_6);

--Merge partitions

ALTER TABLE calendar 
MERGE PARTITIONS PART_2, PART_3 INTO PARTITION PART_7; 

SELECT * FROM calendar PARTITION (PART_7);

--Move partition

ALTER TABLE calendar MOVE PARTITION PART_7 TABLESPACE ts_dw_data_03 NOLOGGING COMPRESS; 

SELECT partition_name, tablespace_name FROM ALL_TAB_PARTITIONS;

--Split partition
ALTER TABLE calendar SPLIT PARTITION PART_7
AT (TO_DATE('01/07/2013', 'DD/MM/YYYY'))
INTO
( PARTITION PART_8, PARTITION PART_9);

SELECT * FROM calendar PARTITION (PART_8);
SELECT * FROM calendar PARTITION (PART_9);

SELECT partition_name, tablespace_name FROM ALL_TAB_PARTITIONS;

--Truncate partition
ALTER TABLE calendar TRUNCATE PARTITION PART_9;

SELECT * FROM calendar PARTITION (PART_9);
--Drop partition
ALTER TABLE calendar DROP PARTITION PART_9;

SELECT partition_name, tablespace_name FROM ALL_TAB_PARTITIONS;

--Add partition

--ALTER TABLE calendar ADD PARTITION PART_8 VALUES LESS THAN (TO_DATE('01/01/2016', 'DD/MM/YYYY')) TABLESPACE ts_dw_data_02;
