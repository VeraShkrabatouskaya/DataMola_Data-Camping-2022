...
CREATE OR REPLACE VIEW v_sal_budgets_customer_brand
AS SELECT
...
GROUP BY TRUNC (fct_b.TIME_ID, 'MM'), ROLLUP (ag.agency_name, cust.customer_name, cust.brand_name)
    HAVING TRUNC (fct_b.TIME_ID, 'MM') IS NOT NULL 
    and ag.agency_name IS NOT NULL 
order by TRUNC (fct_b.TIME_ID, 'MM'), ag.agency_name, cust.customer_name, cust.brand_name;
